...
docker service rm test_airports_1_0_1 > /dev/null 2>&1 &
docker service create --health-cmd "curl http://127.0.0.1:8080/health/ready" --health-interval 15s --health-retries 10 --name test_airports_1_0_1 --mount type=bind,source=/hosthome/shared_drive/airports/,destination=/opc lamersons/airports:assembly-1.0.1
docker service rm test_airports_1_0_1 > /dev/null 2>&1 &'''
          }
        }
...
docker service rm test_airports_1_1_0 > /dev/null 2>&1 &
...
docker service create --health-cmd "curl http://127.0.0.1:8080/health/ready" --health-interval 15s --health-retries 10 --name countries --mount type=bind,source=/hosthome/shared_drive/countries/,destination=/opc lamersons/countries:assembly-1.0.1'''
        ...
        stage('prod_airports_1_0_1') {
          steps {
            sh '''docker pull lamersons/airports:assembly-1.0.1
docker service create --health-cmd "curl http://127.0.0.1:8080/health/ready" --health-interval 15s --health-retries 10 --name prod_airports_1_0_1 --mount type=bind,source=/hosthome/shared_drive/airports/,destination=/opc lamersons/airports:assembly-1.0.1'''
          }
        }
